...
@export var value_to_add: String
@export var value_to_subtract: String
...
func _ready() -> void:
	...
	level = Global[group_name]
	if Global[group_name] > 0:
...
func _on_pressed() -> void:
	...
	if level < max_level:
		...
		if Global.player_money >= int(cost) and level >= levels_till_next_level:
			var_to_change += float(value_to_add)
			var_to_change -= float(value_to_subtract)
